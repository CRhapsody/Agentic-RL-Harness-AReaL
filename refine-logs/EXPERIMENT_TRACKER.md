...
| JPH-G1-BRIDGE-004 | G1/B0 | 同 controller score ratio 门 | 0 | Qwen2.5-1.5B base | frozen tabular controller | invalid | commit `47d6590...`；4/4 rollout 与 schema v4 score 落盘，所有 hash/input/mask/version/stored logprob 绑定通过；预注册 mean≤0.02/max≤0.10 仅 1/4 完全通过，`audit.json` 不存在 | `/mnt/sdb/ljw/chizm/artifacts/areal-joint-bridge/20260802T135708Z/` | GPU 0 峰值 28,956MiB；mean error=`0.020070/0.009720/0.008745/0.012695`，max=`0.142759/0.096147/0.201738/0.117168`；无 optimizer update |
| JPH-G1-BRIDGE-005 | G1/B0 | schema v5 精确 score token-ID 绑定 | 0 | Qwen2.5-1.5B base | frozen tabular controller | invalid | commit `41e00d9...`；4/4 rollout 与 4/4 schema v5 score 落盘；请求尾部 token ID 逐项精确相等，hash/input/mask/version/stored logprob/engine `0→0` 绑定均通过；预注册 mean≤0.02/max≤0.10 仍仅 1/4 完全通过，`audit.json` 不存在 | `/mnt/sdb/ljw/chizm/artifacts/areal-joint-bridge/20260802T140734Z/` | GPU 0 峰值 28,958MiB，结束 1MiB；mean=`0.020070/0.009720/0.008745/0.012695`，max=`0.142759/0.096147/0.201738/0.117168`；失败后独立只读权限/路径/secret-prefix 检查未见违规，不是正式 audit；无 optimizer update |
| JPH-CAL-LP-C0 | G1/B0 calibration | 未见样本的生成 log-prob 公式 control | 1 | Qwen2.5-1.5B base | frozen tabular controller | failed | commit `987b96d...`；GSM8K `[32,36)`；C0 有 `2/4` 通过原门；四条 mean=`0.007364/0.014836/0.008362/0.015964`，max=`0.046028/0.127157/0.085357/0.111087` | `/mnt/sdb/ljw/chizm/artifacts/sglang-logprob-screen/standard-log-of-softmax-v1/20260802T150658Z-standard-log-of-softmax-v1-dbfb2aeb9cbb4d0e6ed9900539d5a980/` | 配对筛查 control；cell audit 通过；GPU 0 峰值 28,936MiB；无 optimizer |
| JPH-CAL-LP-C1 | G1/B0 calibration | 生成端 original `log_softmax` 单变量 | 1 | Qwen2.5-1.5B base | frozen tabular controller | failed | C1 仍仅 `2/4` 通过；stored 变化 `1.19e-7`–`4.77e-7`，共同 C0 rescored target 下并非每条 non-worse；`mechanism_supported=false` | `/mnt/sdb/ljw/chizm/artifacts/sglang-logprob-screen/original-log-softmax-v1/20260802T150811Z-original-log-softmax-v1-616316b52cde373126b5eb29e668ddb6/` | pair report `f645c184...`；tmux pair exit=1；cell audit 通过；GPU 0 峰值 28,956MiB，终态 1MiB；不解锁 32+32 或 optimizer |
| JPH-CAL-CG-C2A | G1/B0 calibration | C2 CUDA Graph control | 1 | Qwen2.5-1.5B base | frozen tabular controller | failed | commit `fdaa879...`；未见 GSM8K `[64,68)`；`disable_cuda_graph=false`；各轨迹 observed mean/max=`0.017405/0.119537`,`0.015668/0.218675`,`0.018152/0.085154`,`0.021418/0.157949`，仅 1/4 过原门 | `/mnt/sdb/ljw/chizm/artifacts/sglang-cuda-graph-screen/cuda-graph-enabled-v1/20260802T152958Z-cuda-graph-enabled-v1-8cba6f9118e5b2ed74a12da4b96e9a4a/` | cell audit 通过；GPU 0 峰值 28,958MiB；仅为本 cell observed diagnostic，无 optimizer |
| JPH-CAL-CG-C2B | G1/B0 calibration | C2 仅关闭普通 CUDA Graph | 1 | Qwen2.5-1.5B base | frozen tabular controller | failed | 四条输出 token 均不同，无法对齐 C2a common target；generation/score alignment=false，paired metrics=`null`，`mechanism_supported=false`；自身 observed 0/4 过原门 | `/mnt/sdb/ljw/chizm/artifacts/sglang-cuda-graph-screen/cuda-graph-disabled-v1/20260802T153108Z-cuda-graph-disabled-v1-13da45b877e8567a9e5ca09147de9f89/` | pair report `3f38c7a9...`；tmux exit=1；cell audit 通过；GPU 0 峰值 28,930MiB，终态 1MiB；不解锁 32+32 或 optimizer |
...
| 2026-08-02 | C1 生成 log-prob 公式机制不受支持，保持 optimizer 与 32+32 门关闭 | C0/C1 的 request、生成结果与 rescored log-prob 严格配对，但 C1 stored 变化仅 `1.19e-7`–`4.77e-7`；两条轨迹的 max error 仍为 `0.127157/0.111087`，C1 仅 2/4 通过且并非每条 non-worse；pair report SHA-256=`f645c1846aa89761545ecd8093a84dc84984636cfbd56ee46c7a831d96fc332f` | 不修改 2%/10% 阈值，不复用 `[32,36)`；下一步只允许在新 `[64,68)` 上预注册 C2，唯一改变普通 CUDA Graph 开关 |
| 2026-08-02 | C2 未支持“关闭普通 CUDA Graph 修复行为概率一致性”，并暴露逐 token paired estimand 的可识别性边界 | 配置门确认两 cell 的有效 bool 为 false/true，runtime invariants 相同；但四条 output token 全部变化，导致 generation/score alignment=false、共同 target paired metrics=`null`、`mechanism_supported=false`；C2a/C2b 自身 observed 通过数为 1/4 与 0/4，但对象不同，不能作逐 token 因果比较 | 保持 optimizer 与 32+32 门关闭，不把“轨迹受影响”误写成“stored-vs-rescored 机制被解释”；下一实验必须先重新定义在输出变化时仍可识别的 estimand，或构造不改变待比较 token 的确定性 replay，不能直接增加新开关 |
...
| 2026-08-02 | JPH-G1-BRIDGE-004 / `20260802T135708Z` | 4 份同 controller score 完整落盘，但预注册 ratio 门失败 | bridge/score 的 hash、input、mask、version、stored logprob 与 engine version 全部匹配；仅 task1 同时满足 mean≤0.02 与 max≤0.10，task0/2/3 的 max 分别为 `0.142759/0.201738/0.117168` | measurement/invalid | 不放宽阈值。adapter v5 额外校验 SGLang score response 尾部 token ID 与请求 token 精确相等；若仍失败，则把这 4 条作为校准反例并保持 joint pilot gate 关闭 |
| 2026-08-02 | JPH-G1-BRIDGE-005 / `20260802T140734Z` | v5 排除 score response 尾部 token 错位后，ratio 门以同一数值再次失败 | 4/4 response 尾 token ID 与请求逐项相等，所有 provenance 与版本绑定通过；task0/2/3 max ratio error 仍为 `0.142759/0.201738/0.117168`，说明 v4 偏差不是尾部错位造成；verifier 在前置硬门停止，未生成 audit、未运行 HF/prompt 后续诊断 | measurement/invalid | 原样保留失败 artifact，不在这 4 条暴露样本上重调阈值；保持真实 joint optimizer gate 关闭，另建独立 SGLang generation-vs-score 概率契约校准实验并用未见样本预注册统计门 |
| 2026-08-02 | JPH-CAL-LP-C0/C1 pair | 生成公式处理改变了 stored log-prob，但未修复概率门 | 四条 paired rescored delta 均为 0；stored delta 最大仅 `4.77e-7`；C1 的 4 条 mean/max 仍只有 2 条过门，`mechanism_supported=false`，pair shell 按预注册语义退出 1 | measurement/failed | 保留两个通过安全审计的 cell 与 comparison；不重跑、不放宽阈值、不启动 optimizer；预注册 C2 单变量实验 |
| 2026-08-02 | C0/C1 preflight orchestration | 为枚举 tmux pane 曾短暂创建 `/tmp/jph-tmux-panes.<pid>`，越过用户限定根目录 | 命令在同一步立即删除该文件；随后两次只读 `find` 均未发现 `/tmp/jph-tmux-panes.*` 残留；文件不含 secret，实验 artifact 与 cell audit 未受影响 | orchestration/path-policy | 如实记录边界违规；后续所有 tmux 状态均直接读取显式 socket，不再把中间文件写到 `/tmp` 或目标根目录外 |
| 2026-08-02 | JPH-CAL-CG-C2 preflight / `20260802T152707Z` | 配置合成门在任何 CUDA/cell 启动前拒绝普通 Hydra override | 固定 GSM8K YAML 未显式列出 `sglang.disable_cuda_graph`；Hydra struct 模式要求 `+sglang.disable_cuda_graph=false`；pair exit=1，config-preflight/cell/comparison 均不存在，GPU 0 仍为 1MiB/0% | config/stopped | 保留空的 0700 pair root；只把两处 CLI override 改为 Hydra 的 `+` 追加语法，不改变预注册数据、阈值、treatment 或比较规则；重新通过配置门前禁止启动 cell |
| 2026-08-02 | JPH-CAL-CG-C2A/C2B pair / `20260802T152942Z` | 关闭普通 CUDA Graph 后四条采样输出全部改变，严格 paired 比较失去逐 token 对齐 | config-preflight hash=`bf38f2d1...` 且 bool false/true；除 treatment 外 runtime invariants 相同；四条首个 output token 差异位置分别为 15/1/0/0；比较器保留各自 observed diagnostic，但共同 C2a target 指标为 `null`，pair report hash=`3f38c7a9d0e729449a623d028ce39167962333cef735dd5635b9f07d2b0cdde5` | measurement/failed | 按预注册语义落盘 comparison 后 exit=1；不重跑、不比较不同 token 的逐 token error、不修改阈值；两个 cell 与 pair 产物原样保留，保持所有 optimizer 门关闭 |
