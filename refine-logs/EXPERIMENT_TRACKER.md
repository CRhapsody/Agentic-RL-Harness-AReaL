...
| JPH-CAL-LP-C0 | G1/B0 calibration | 未见样本的生成 log-prob 公式 control | 1 | Qwen2.5-1.5B base | frozen tabular controller | planned | GSM8K test 顺序位置 `[32,36)`；standard `log(softmax)`；原 2%/10% 门不变 | 外置 `artifacts/sglang-logprob-screen/` | 机制筛查；无 optimizer；必须与 C1 exact-paired |
| JPH-CAL-LP-C1 | G1/B0 calibration | 生成端 original `log_softmax` 单变量 | 1 | Qwen2.5-1.5B base | frozen tabular controller | planned | 与 C0 唯一差异为整服启动前 `SGLANG_RETURN_ORIGINAL_LOGPROB=1`；需 4/4 通过且 paired non-worse | 外置 `artifacts/sglang-logprob-screen/` | 通过也只解锁 32+32 校准/确认，不解锁 optimizer |
