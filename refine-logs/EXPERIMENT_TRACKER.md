...
| JPH-CAL-CG-C2A | G1/B0 calibration | C2 CUDA Graph control | 1 | Qwen2.5-1.5B base | frozen tabular controller | planned | 新未见 GSM8K `[64,68)`；standard log-prob；`disable_cuda_graph=false`；原 2%/10% 门不变 | 外置 `artifacts/sglang-cuda-graph-screen/` | 必须与 C2B 同 GPU 串行 exact-paired；无 optimizer |
| JPH-CAL-CG-C2B | G1/B0 calibration | C2 仅关闭普通 CUDA Graph | 1 | Qwen2.5-1.5B base | frozen tabular controller | planned | 与 C2A 唯一处理差异为 `disable_cuda_graph=true`；需逐轨迹 non-worse、至少一条改善且 4/4 过原门 | 外置 `artifacts/sglang-cuda-graph-screen/` | 通过也只允许另行预注册 32+32；不直接解锁 optimizer |
