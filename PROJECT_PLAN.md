...
本项目不再设置每卡新增显存的固定软/硬上限，可按实验需要使用当前可用显存。任何 GPU 命令启动前仍必须重新读取 `memory.used`、`memory.free` 与现有计算进程，并把启动前基线和运行中采样写入项目外运行目录。正常情况下与现有进程共存；只有实际显存不足或启动因显存失败时，才允许在确认账号身份后终止用户 `lyy` 的进程并重新采集资源状态。任何其他用户或项目进程一律不得终止。
