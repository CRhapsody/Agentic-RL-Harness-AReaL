...
logger = logging.getLogger("JPHAReaLTraceEval")


def _task_limit() -> int:
    raw = os.environ.get("JPH_AREAL_TRACE_TASKS", "1")
    try:
        value = int(raw)
    except ValueError as exc:
        raise ValueError(f"JPH_AREAL_TRACE_TASKS must be an integer: {raw}") from exc
    if value < 1 or value > 8:
        raise ValueError(f"JPH_AREAL_TRACE_TASKS must be between 1 and 8: {value}")
    return value


def main(args: list[str]) -> None:
    config, _ = load_expr_config(args, GRPOConfig)
    trace_dir = Path(os.environ["JPH_AREAL_TRACE_DIR"]).resolve()
    logging.setup_file_logging(str(trace_dir.parent / "areal-eval.log"))

    tokenizer = load_hf_tokenizer(config.tokenizer_path)
    seeding.set_random_seed(config.seed, key="jph-areal-trace-eval")
    ...
    results = []
    try:
        controller.initialize(role="jph-trace-rollout", server_args=server_args)
        workflow_kwargs = {
        ...
        for batch in dataloader:
            for item in batch:
                controller.submit(
                    item,
                    workflow="jphrl.areal_trace_workflow.ArealTraceRLVRWorkflow",
        ...
        logger.info("AReaL trace evaluation: %s", tabulate_stats(controller.export_stats()))
    finally:
        controller.destroy()

    trace_files = sorted(trace_dir.glob("trace-*.json"))
    if len(trace_files) != submitted:
        raise RuntimeError(
            f"expected {submitted} trace files, found {len(trace_files)} in {trace_dir}"
        )
    print(
        f"AReaL trace generation complete: submitted={submitted} "
        f"accepted={len(results)} traces={len(trace_files)}"
    )
